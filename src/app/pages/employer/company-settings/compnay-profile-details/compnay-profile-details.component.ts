...
import { listIndustry, SIGN_UP_STEP } from 'src/app/constants/config';
...
@Component({
  selector: 'ms-compnay-profile-details',
  templateUrl: './compnay-profile-details.component.html',
  styleUrls: ['./compnay-profile-details.component.scss']
})
export class CompnayProfileDetailsComponent implements OnInit {
  ...
  setData(data) {
    ...
    this.formUpdate.controls['companyName'].setValue(user.company_name || '');
    ...
    if (user.company_profile_picture) {
      this.formUpdate.controls['companyProfilePicture'].setValidators([]);
    }
  ...
  checkUserMember(user) {
    return user.employer_id && user.accountType == 0;
  }
  ...
  save(form) {
    ...
    let formData = {
      ...
      employer_company_url: this.isCompanySettings ? form.employerCompanyUrl : this.companyWebsite,
      employer_company_facebook: this.isCompanySettings ? form.employerCompanyFacebook : this.companyFacebook,
      employer_company_twitter: this.isCompanySettings ? form.employerCompanyTwitter : this.twitterPage,
      ...
      sign_up_step: SIGN_UP_STEP.STEP2,
    ...
    this.userService.updateUserProfile(formData).subscribe(res => {
      ...
      if (this.isCompanySettings) this.next.emit();
    }, errorRes => {
      this.isCallingApi = false;
  ...
  async deleteCompanyPhoto(id?: number) {
    ...
    if (isConfirmed) {
      ...
      const data = {
        path: id === null ? this.ceoPicture : photoItem?.url
      }
      console.log(data);
